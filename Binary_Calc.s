...
main:	@ Main calls our menu function and leaves
	@ldr r0, = startthumb + 1
	@bx  r0
...
@startthumb:
	...
	ldr	r2, = Menu
        bl      r2			@ Branch with link to Menu 

@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@

Menu:	@ Menu takes two user input Hex numbers and stores to RAM,
	@ then it presents the user the menu choices and paths the user to
	@ their chosen operation.

        push    {r7, lr}		@ Push LR and r7 onto stack
        mov     r7, sp			@ Move SP to r7
        sub     sp, sp, #56		@ Creating space on the stack
        ldr     r0, = sWelcome		@ Load greeting msg into r0 
        bl      printf			@ Print msg to screen
	ldr     r1, = sUserHexInput	@ Load string for user input into r1
        ...
        bl      scanf			@ call scanf for first user hex num
	
        sub     r1, r7, #8		@ creates space between r7 for r1
	mov	r0, r5			@ copy r5 over r0			
        bl      scanf			@ call scanf for second user hex num

        ldr     r1, = sMenuOptions	@ load menu options string into r1
        mov     r0, r1			@ mov r1 over r0
        bl      printf			@ call printf to print menu options to screen
	...
	ldr	r6, = #12
        ...
        ldr     r1, [r7, #-12]		@ Load user input to r1 grabbing (r7 -12)
        ...
        ldr     r0, [r7, #-4]		@ loads r7 -4 for hex 1 user num into r0
        ldr     r1, [r7, #-8]		@ loads r7 -8 for hex 2 user num into r1
	ldr	r3, = BinaryAnd
        bl      r3			@ Branch with link to Binary And for calculation

CompareIfTwo:
        ldr     r0, [r7, #-12]		@ Load user input to r0 grabbing (r7 -12)
        ...
        bl      printf			@ Display message to screen
        ldr     r0, [r7, #-4]		@ loads r7 -4 for hex 1 user num into r0
        ldr     r1, [r7, #-8]		@ loads r7 -8 for hex 2 user num into r1
	ldr	r3, = BinaryOr
        bl     r3			@ Branch with link to Binary Or for calculation
...
CompareIfThree:
        ...
        ldr     r1, [r7, #-8]		@ loads r7 -8 for hex 2 user num into r1
	ldr	r3, = BinaryXor
        bl     r3			@ Branch with link to Binary Xor for calculation
...
CompareIfFour:
        ...
        ldr     r1, [r7, #-8]		@ loads r7 -8 for hex 2 user num into r1
	ldr 	r3, = BinaryBic
        bl     r3			@ Branch with link to Binary Bic for calculation

BadInput:
	ldr 	r0, = sMenuBadInput		@ If match not found load message to chatise the user to try again 
	bl 	printf			@ print the chastisement to the screen so the user may feel bad
	ldr	r3, = Menu
	bl     r3			@ If user input isn't 1-4 go back to start of Menu
...
BinaryAnd:	@ All the Binary functions are total copies of each other one except for which registers they push and which
	...
	mov	r2, r6			@ mov r6 to r2 in order to print hex num 2 as part of answer
        ...
        bl      printf			@ print the message to the screen where r0 is the message, r1 is hex num 1, r2 
        ...
        bl      Menu			@ If it does match, branch to Menu
...
BinaryBic:@ All the Binary functions are total copies of each other one except for which registers they push and which
	...
	mov	r5, r0			@ Copy r0 which holds Hex num 1 into r5 to keep copy after bitwise op
	mov	r6, r1			@ Copy r1 which holds Hex num 2 into r6 to keep copy after bitwise op		
        bic     r0, r0, r1		@ Perform Bic bitwise op on r1 and r0 and store result in r0
	mov	r3, r0			@ Copy r0 to r3 to get in correct register for print statement
